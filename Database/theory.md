...
> 위 그림에서는 SELECT, UPDATE 작업이 성공적으로 수행되었으나 중간에 실패한 작업(다른 계좌로 송금)이 발생하여 트랜잭션이 ROLLBACK되어 모든 작업이 원래 상태로 돌아가면서 데이터베이스 일관성을 유지할 수 있음
...
- **단점**
    - **페이지 분할 발생**: 새로운 데이터가 추가될 때 기존 데이터의 절반이 새로운 페이지로 이동되어 성능이 저하될 수 있음

**비클러스터드 인덱스**

![](./image/Non-Clustered_Index.png)
> 위 그림은 비클러스터드 인덱스의 구조로 Root Level은 인덱스 키와 해당 데이터 페이지의 위치를 저장하고, Leaf Level은 데이터 페이지의 포인터를 포함하며 데이터는 실제로 별도의 Data Page에 저장됨
...
> 위 그림은 B-Tree의 구조로 B-Tree의 각 노드는 여러 개의 키를 포함할 수 있으며 자식 노드로 연결됨
...
> 위 그림은 Hashing의 작동 방식으로 해시 함수에 의해 특정 해시값으로 변환된 후 버킷에 저장된 데이터에 매핑됨 
...
![](./image/Vertical_Partitioning.png)
